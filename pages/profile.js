...
export default function Profile(uid) {
 ...
 function field({ label, type, name, useDatalist, id, arr }) {
  ...
  let input = cEl('input', { ariaLabel: label, class: 'w-full p-3 bg-7 color2', type, disabled: true, name: id || name, placeholder: label, event: {
   change() {
    
   }
  } });

  useDatalist && input.setAttribute('list', id);
