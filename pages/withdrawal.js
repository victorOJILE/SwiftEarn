...
const balanceIcon = `<svg class="text-blue-200" stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 16 16" width="2.2em" height="2.2em" xmlns="http://www.w3.org/2000/svg">
 <path d="M5.5 9.511c.076.954.83 1.697 2.182 1.785V12h.6v-.709c1.4-.098 2.218-.846 2.218-1.932 0-.987-.626-1.496-1.745-1.76l-.473-.112V5.57c.6.068.982.396 1.074.85h1.052c-.076-.919-.864-1.638-2.126-1.716V4h-.6v.719c-1.195.117-2.01.836-2.01 1.853 0 .9.606 1.472 1.613 1.707l.397.098v2.034c-.615-.093-1.022-.43-1.114-.9H5.5zm2.177-2.166c-.59-.137-.91-.416-.91-.836 0-.47.345-.822.915-.925v1.76h-.005zm.692 1.193c.717.166 1.048.435 1.048.91 0 .542-.412.914-1.135.982V8.518l.087.02z" />
 <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z" />
 <path d="M8 13.5a5.5 5.5 0 1 1 0-11 5.5 5.5 0 0 1 0 11zm0 .5A6 6 0 1 0 8 2a6 6 0 0 0 0 12z" />
</svg>`;

const totalEarningIcon = `<svg class="text-blue-200" stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 16 16" width="2.2em" height="2.2em" xmlns="http://www.w3.org/2000/svg">
 <path d="M5 8a2 2 0 1 0 0-4 2 2 0 0 0 0 4Zm4-2.5a.5.5 0 0 1 .5-.5h4a.5.5 0 0 1 0 1h-4a.5.5 0 0 1-.5-.5ZM9 8a.5.5 0 0 1 .5-.5h4a.5.5 0 0 1 0 1h-4A.5.5 0 0 1 9 8Zm1 2.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 0 1h-3a.5.5 0 0 1-.5-.5Z" />
 <path d="M2 2a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2H2ZM1 4a1 1 0 0 1 1-1h12a1 1 0 0 1 1 1v8a1 1 0 0 1-1 1H8.96c.026-.163.04-.33.04-.5C9 10.567 7.21 9 5 9c-2.086 0-3.8 1.398-3.984 3.181A1.006 1.006 0 0 1 1 12V4Z" />
</svg>`;

const infoIcon = `<svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 512 512" width="2.8rem" height="2.8rem"  xmlns="http://www.w3.org/2000/svg">
 <path d="M256 8C119.043 8 8 119.083 8 256c0 136.997 111.043 248 248 248s248-111.003 248-248C504 119.083 392.957 8 256 8zm0 110c23.196 0 42 18.804 42 42s-18.804 42-42 42-42-18.804-42-42 18.804-42 42-42zm56 254c0 6.627-5.373 12-12 12h-88c-6.627 0-12-5.373-12-12v-24c0-6.627 5.373-12 12-12h12v-64h-12c-6.627 0-12-5.373-12-12v-24c0-6.627 5.373-12 12-12h64c6.627 0 12 5.373 12 12v100h12c6.627 0 12 5.373 12 12v24z" />
</svg>`;

export default function Withdrawal(uid) {
 const acctBalance = cEl('span', { class: 'text-xl text-gray-100 font-bold', textContent: '...' });
 const totalEarning = cEl('span', { class: 'text-xl text-gray-100 font-bold', textContent: '...' });
 ...
 const main = cEl('main', { class: 'p-3 md:p-6 bg-9 color2 overflow-auto md:h-screen container mx-auto' },
  cEl('div', { class: 'max-w-xl' },
   cEl('h2', { class: 'text-2xl md:text-3xl mb-2', textContent: 'Fund Withdrawal' }),
   cEl('p', { class: 'text-sm color4', textContent: 'Experience hassle-free fund withdrawals, transparent transaction processes, and swift payouts. Request withdrawals, keep track of your financial activities, and conveniently manage your funds.' })
  ),
  cEl('section', { class: 'py-2 mb-8 grid sm:grid-cols-2 items-center gap-8 md:my-8' },
   cEl('div', { class: 'text-center my-4 relative' },
    cEl('a', {
      event: {
       click: async function(e) {
        e.preventDefault();
        let cover = cEl('div', { class: 'w-full h-full top-0 absolute bg-white opacity-20' });
        this.parentElement.append(cover);
        
        let comp = await import('../components/withdrawal.js');
        comp = comp.default(uid);
        main.append(
         comp,
         cEl('div', { class: 'max-w-xl mx-auto my-6 p-3 bg-yellow-500 text-yellow-50 text-sm border flex' },
          svg(infoIcon),
          cEl('p', { textContent: 'Please note that withdrawals may take 3-5 business days to be processed!', class: 'ml-3' })
         )
        );
        comp.scrollIntoView();
        
        this.lastElementChild.textContent = 'Please, fill the form below!';
        
       }
      },
      href: '',
      class: 'block p-4 bg-green-900 rounded-lg border-2 border-green-700 text-green-100 font-bold'
     },
     svg(moneybag),
   ...
   cEl('div', {},
    cEl('div', { class: 'flex items-center bg-blue-900 mb-8 py-4 px-3 rounded-xl', style: { boxShadow: '0px 0px 2px 5px #313B89' } },
     svg(totalEarningIcon),
     cEl('div', { class: 'pl-3' },
      totalEarning,
