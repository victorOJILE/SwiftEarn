...
import { useState, useLayoutEffect, useRef, lazy } from 'react';
import Transactions from '../components/transactions.js';
import { useAuthContext } from '../pages/_app.js';

const WithdrawalComp = lazy(() => import('../components/withdrawal.js'));
...
const balanceIcon = <svg className="text-blue-200" stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 16 16" width="2.2em" height="2.2em" xmlns="http://www.w3.org/2000/svg">
 <path d="M5.5 9.511c.076.954.83 1.697 2.182 1.785V12h.6v-.709c1.4-.098 2.218-.846 2.218-1.932 0-.987-.626-1.496-1.745-1.76l-.473-.112V5.57c.6.068.982.396 1.074.85h1.052c-.076-.919-.864-1.638-2.126-1.716V4h-.6v.719c-1.195.117-2.01.836-2.01 1.853 0 .9.606 1.472 1.613 1.707l.397.098v2.034c-.615-.093-1.022-.43-1.114-.9H5.5zm2.177-2.166c-.59-.137-.91-.416-.91-.836 0-.47.345-.822.915-.925v1.76h-.005zm.692 1.193c.717.166 1.048.435 1.048.91 0 .542-.412.914-1.135.982V8.518l.087.02z" />
 <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z" />
 <path d="M8 13.5a5.5 5.5 0 1 1 0-11 5.5 5.5 0 0 1 0 11zm0 .5A6 6 0 1 0 8 2a6 6 0 0 0 0 12z" />
</svg>;

const totalEarningIcon = <svg className="text-blue-200" stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 16 16" width="2.2em" height="2.2em" xmlns="http://www.w3.org/2000/svg">
 <path d="M5 8a2 2 0 1 0 0-4 2 2 0 0 0 0 4Zm4-2.5a.5.5 0 0 1 .5-.5h4a.5.5 0 0 1 0 1h-4a.5.5 0 0 1-.5-.5ZM9 8a.5.5 0 0 1 .5-.5h4a.5.5 0 0 1 0 1h-4A.5.5 0 0 1 9 8Zm1 2.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 0 1h-3a.5.5 0 0 1-.5-.5Z" />
 <path d="M2 2a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2H2ZM1 4a1 1 0 0 1 1-1h12a1 1 0 0 1 1 1v8a1 1 0 0 1-1 1H8.96c.026-.163.04-.33.04-.5C9 10.567 7.21 9 5 9c-2.086 0-3.8 1.398-3.984 3.181A1.006 1.006 0 0 1 1 12V4Z" />
</svg>;

const infoIcon = <svg stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 512 512" width="2.8rem" height="2.8rem" xmlns="http://www.w3.org/2000/svg">
...
export default function Withdrawal({ uid }) {
 ...
 return (
 	<>
 		<Head>
 			<title>SwiftEarn - Fund Withdrawal</title>
				<meta name="description" content="Effortlessly withdraw and manage your earnings. Enjoy a seamless withdrawal experience on our platform." />
 		</Head>
  <main className="p-3 md:p-6 bg-9 color2 overflow-auto md:h-screen container mx-auto">
   <div className="max-w-xl">
    <h2 className="text-2xl md:text-3xl mb-2">Fund Withdrawal</h2>
    <p className="text-sm color4">Experience hassle-free fund withdrawals, transparent transaction processes, and swift payouts. Request withdrawals, keep track of your financial activities, and conveniently manage your funds.</p>
   </div>
   <section className="py-2 mb-8 grid sm:grid-cols-2 items-center gap-8 md:my-8">
    <div className="text-center my-4 relative">
     <div className="block p-4 bg-green-900 rounded-lg border-2 border-green-700 text-green-100 font-bold" onClick={e => {
       setDisplayCover(true);
       
       e.target.lastElementChild.textContent = 'Please, fill the form below!';
     }}>
      {svg(moneybag)}
      <span className="p-2 inline-block py-4 text-sm">Request Withdrawal</span>
     </div>
     {
      displayCover && <div className="w-full h-full top-0 absolute bg-white opacity-20"></div>
     }
    </div>
    <div>
     <Stats icon={totalEarningIcon} text="Total Earnings" value={nbf.format(data.totalEarning || 0) + ' 💸'} />
     <Stats icon={balanceIcon} text="Current Balance" value={nbf.format(data.balance || 0) + ' 💸'} />
    </div>
   </section>
   <Transactions uid={uid} />
  	{ displayCover && <WithdrawalComp uid={uid} ref={form} /> }
  	{ displayCover && (
  		<div className="max-w-xl mx-auto my-6 p-3 bg-yellow-500 text-yellow-50 text-sm border flex">
  		 {infoIcon}
  			<p className="ml-3">Please note that withdrawals may take 3-5 business days to be processed!</p>
  		</div>
  	) }
  </main>
  </>
