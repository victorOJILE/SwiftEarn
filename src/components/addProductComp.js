...
import { getFirestore, getDoc, getDocs, setDoc, doc, query, collection, where } from 'https://www.gstatic.com/firebasejs/10.0.0/firebase-firestore.js';*/
...
export default function AddProduct(uid, productData) {
	...
	const section = cEl('section', 
		...
		cEl('form', {
				name: 'setting',
				class: 'text-xs', autoComplete: true,
				event: {
					submit: function(e) {
						e.preventDefault();
						...
						let data = edit && productData || {
							...
							sales: 0,
							conversion: '0%',
							vendor_name: 'Shanks George' || username,
							product_id,
							status: 'Pending',
						...
						let productImg = this.productImageUrl.files[0];
						...
						sendRequest(data).then(res => {
								// Upload image
								if (productImg.type.match(new RegExp('image.*')) && productImg.type.match(new RegExp('jpg||JPG||png||PNG||gif||GIF||webp'))) {
									...
									uploadBytes(imageRef, productImg).then((snapshot) => {
											getDownloadURL(imageRef)
												.then(url => {
													sendRequest({
															// Add productImageUrl to vendor data
															productImageUrl: url
														})
							...
							event: {
								change: function() {
									try {
										...
										if (!file.type.match(new RegExp('image.*')) && file.type.match(new RegExp('jpg||JPG||png||PNG||gif||GIF||webp'))) {
											return alert('Please upload an image file of these formats: .jpg .JPG .png .PNG .gif .GIF .webp');
										}
