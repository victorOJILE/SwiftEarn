...
export default function highDemandProducts(products) {
	...
	if(!products || !products.length) {
		...
		const buttonLeft = cEl('button', { class: 'absolute top-1/2 transform -translate-y-1/2 w-8 h-8 rounded-full bg-white bg-opacity-50 text-black text-2xl outline-none cursor-pointer left-0' }, svg(icons.chevronLeft));
		const buttonRight = cEl('button', { class: 'absolute top-1/2 transform -translate-y-1/2 w-8 h-8 rounded-full bg-white bg-opacity-50 text-black text-2xl outline-none cursor-pointer right-0' }, svg(icons.chevronRight));
...
function generateHighDemandProducts(product) {
	return cEl('li', { className: 'm-2' },
			...
			cEl('a', { href: product.href || '' },
				...
				cEl('p', { className: 'text-xs text-gray-400', textContent: product.vendor || '' }),
				cEl('span', { className: 'font-bold', textContent: product.currentPrice || '' }),
				cEl('span', { className: 'px-2 text-gray-400' },
					cEl('span', { textContent: '|' })
				),
				cEl('span', { className: 'text-xs text-yellow-600', textContent: (product.commission || '') + '% comm' })
			)
		);
}
