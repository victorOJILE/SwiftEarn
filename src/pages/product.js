...
import { clock, longArrowRight, copy } from '../icons.js';
...
function ProductComp(uid) {
 ...
 if(product_id) product_id = decodeURIComponent(product_id);
       ...
       cEl('div', { class: 'grid md:grid-cols-2 md:gap-6' },
        ...
        cEl('div', 
         ...
         cEl('div', { class: 'flex items-center text-yellow-600 text-xs' },
          svg(clock), document.createTextNode('We have limited time left!')
         ),
         cEl('div', { class: 'mt-4 mb-8' },
          cEl('a', { href: data.jvPageUrl, class: 'underline flex items-center text-green-500' },
           document.createTextNode('Check JV Page '), svg(longArrowRight)
          )
         )
        )
       ),
...
unsubscribe.authenticate = function(uid) {
 Header('Marketplace', uid).append(ProductComp(uid));
}